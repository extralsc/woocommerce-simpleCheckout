 ...
 */
class SimpleCheckout_Enqueue extends SimpleCheckout
{
	...
	public function enqueue_require_bundle()
	{
		...
		if (!wp_script_is('require-bundle', 'registered')) {
			wp_enqueue_script('require-bundle', self::$directory_url . 'js/lib/require-bundle' . self::$asset_version . '.js', NULL, NULL);
		}
		if (!wp_script_is('require-polyfills', 'registered')) {
			wp_enqueue_script('require-polyfills', self::$directory_url . 'js/lib/require-polyfills' . self::$asset_version . '.js', NULL, NULL);
		}
	}
	...
	{
		// Bail if not visiting pages affected by the plugin
		if (is_admin() || !function_exists('is_account_page') || !is_account_page() || !is_wc_endpoint_url('edit-address')) {
			return;
		}
	...
	public function enqueue_theme_compat_styles()
	{
		...
		foreach ($theme_slugs as $theme_slug) {
			...
			if (apply_filters('sc_enable_compat_theme_style_' . $theme_slug, true) === false) {
				continue;
			}
	...
	public function enqueue_plugin_compat_styles()
	{
		...
		foreach ($plugins_installed as $plugin_file => $plugin_meta) {
			...
			// Maybe skip compat file
			if (apply_filters('sc_enable_compat_plugin_style_' . $plugin_slug, true) === false) {
				continue;
			}
...
SimpleCheckout_Enqueue::instance();
