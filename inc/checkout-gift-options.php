 ...
 */
class SimpleCheckout_GiftOptions extends SimpleCheckout
{
	...
	public function hooks()
	{
		...
		// Checkout
		add_action('sc_output_step_shipping', array($this, 'output_substep_gift_options'), 90);
		...
		add_filter('sc_customer_persisted_data_clear_fields_order_processed', array($this, 'change_customer_persisted_data_clear_fields_order_processed'), 10);
		add_filter('sc_customer_persisted_data_clear_all_fields_skip_list', array($this, 'change_customer_persisted_data_clear_fields_skip_list'), 10);
		...
		add_filter('sc_hide_optional_fields_skip_list', array($this, 'prevent_hide_optional_fields_gift_options'), 10);
		...
		// Packing slips
		add_filter('sc_packing_slips_message_box_args', array($this, 'change_packing_slip_message_box_args'), 10, 2);
	}
	...
	public function output_substep_gift_options($step_id)
	{
		...
		$substep_title = __('Gift options', 'simple-checkout');
	...
	public function get_substep_text_gift_options()
	{
		...
		return apply_filters('sc_substep_gift_options_text', $html);
	}
	...
	public function get_gift_options_fields()
	{
		...
		$message_maxlength = apply_filters('sc_gift_options_message_length', false);
		$gift_option_fields = array(
			'_sc_gift_from' => array(
				'type'          => 'text',
				'class'         => array('form-row-wide '),
				'label'         => _x('From', 'Field label for person sending the gift', 'simple-checkout'),
				'placeholder'   => __('Your name', 'simple-checkout'),
				'description'   => __('Name of who is sending the gift, printed on the packing slip.', 'simple-checkout'),
				'default'		=> is_checkout() ? $customer->get_display_name() : null,
				'maxlength'		=> apply_filters('sc_gift_options_from_length', false),
				'custom_attributes' => array(
					'data-autofocus' => true,
				),
			),

			'_sc_gift_message' => array(
				'type'          => 'textarea',
				'class'         => array('form-row-wide '),
				'label'         => __('Gift message', 'simple-checkout'),
				'placeholder'   => __('Write a gift message...', 'simple-checkout'),
				'description'   => $message_maxlength ? sprintf(__('Brief message with up to %d characters, printed on the packing slip.', 'simple-checkout'), $message_maxlength) : __('Brief message, printed on the packing slip.', 'simple-checkout'),
				'default'		=> is_checkout() ? $checkout->get_value('_sc_gift_message') : null,
				'maxlength'		=> $message_maxlength,
			),
		);

		return apply_filters('sc_gift_options_fields_args', $gift_option_fields);
	}
	...
	{
		return array('_sc_gift_message', '_sc_gift_from');
	}
	...
	{
		$skip_list = array_merge($skip_list, array('_sc_gift_message', '_sc_gift_from'));
		return $skip_list;
	}
	...
	public function update_order_meta_with_gift_options_fields($order_id)
	{
		...
		$gift_message = wc_clean(wp_unslash($_POST['_sc_gift_message']));
		...
		foreach ($gift_options_fields as $key => $field) {
			...
			if ($key === '_sc_gift_from' && empty($gift_message)) {
				$field_value = null;
			}
...
?>
		...
		<div class="order_data_column" style="width: 100%">
			...
			<h4><?php echo esc_html(__('Gift options', 'Title for gift options on admin order details screen', 'simple-checkout')); ?> <a href="#" class="edit_address"><?php echo esc_html(_x('Edit', 'Edit gift options link on admin order details screen', 'simple-checkout')); ?></a></h4>
...
<?php
	...
	public function save_order_gift_details($order_id)
	{
		...
		$gift_message = wc_clean(wp_unslash($_POST['_sc_gift_message']));
		$gift_from = wc_clean(wp_unslash($_POST['_sc_gift_from']));

		if (!empty($gift_message)) {
			update_post_meta($order_id, '_sc_gift_message', $gift_message);
			update_post_meta($order_id, '_sc_gift_from', $gift_from);
		} else {
			delete_post_meta($order_id, '_sc_gift_message');
			delete_post_meta($order_id, '_sc_gift_from');
		}
	}
	...
	{
		return get_option('sc_display_gift_message_in_order_details', 'no') == 'yes';
	}
	...
	public function maybe_add_gift_message_order_received_details_table($total_rows, $order, $tax_display)
	{
		...
		// Get gift message value
		$gift_message = $gift_options['_sc_gift_message'];
		$gift_from = $gift_options['_sc_gift_from'];
		...
		if (!empty($gift_from)) {
			$new_total_rows['gift_message_from'] = array(
				'label' => __('Gift message from:', 'simple-checkout'),
				'value' => $gift_from,
			);
		}
	...
	public function output_gift_message_order_details($order)
	{
		...
		if ((!function_exists('is_checkout') || !is_checkout() && !is_order_received_page()) && !is_wc_endpoint_url('view-order')) {
			return;
		}
		...
		if (!array_key_exists('_sc_gift_message', $gift_options) || empty($gift_options['_sc_gift_message'])) {
			return;
		}
	...
	public function add_email_styles($css, $email = null)
	{
		...
		if (null !== $email && !in_array($email->id, $allowed_email_ids)) {
			return $css;
		}
	...
	public function change_packing_slip_message_box_args($args, $order_id)
	{
		...
		if (empty($gift_options['_sc_gift_message'])) {
			return $args;
		}
		...
		$args['message_body'] = $gift_options['_sc_gift_message'];
		$args['message_footer'] = $gift_options['_sc_gift_from'];
...
SimpleCheckout_GiftOptions::instance();
