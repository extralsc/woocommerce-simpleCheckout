 ...
 */
class SimpleCheckout_Validation extends SimpleCheckout
{
	...
	public function hooks()
	{
		...
		add_filter('sc_js_settings', array($this, 'add_checkout_validation_js_settings'), 10);

		// Mailcheck validation
		add_filter('sc_checkout_field_args', array($this, 'change_checkout_email_field_args'), 10);
	...
	public function add_checkout_validation_js_settings($settings)
	{
		...
		$settings['checkoutValidation'] = apply_filters('sc_checkout_validation_script_settings', array(
			...
			'validationMessages' => array(
				...
				'email'                       => __('This is not a valid email address.', 'simple-checkout'),
				'confirmation'                => __('This does not match the related field value.', 'simple-checkout'),
			),
		));
	...
	public function change_checkout_email_field_args($field_args)
	{
		...
		$checkout_email_fields = apply_filters('sc_checkout_email_fields_for_mailcheck', array('billing_email'));
	...
	public function add_checkout_field_validation_status_classes($args, $key, $value)
	{
		...
		if (array_key_exists('sc_skip_server_validation', $args) && true === $args['sc_skip_server_validation']) {
			return $args;
		}
	...
	public function change_required_field_attributes($field, $key, $args, $value)
	{
		...
		$field = str_replace('<abbr class="required"', '<abbr class="required" aria-label="' . __('(Required)', 'simple-checkout') . '" ', $field);
...
SimpleCheckout_Validation::instance();
