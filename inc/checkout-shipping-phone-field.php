 ...
 */
class SimpleCheckout_CheckoutShippingPhoneField extends SimpleCheckout
{
	...
	public function hooks()
	{
		...
		add_filter('sc_substep_shipping_address_text', array($this, 'add_shipping_phone_to_substep_text_format'), 10);
		...
		if ('contact' === get_option('sc_shipping_phone_field_position', 'shipping_address')) {
			// Remove shipping phone field from shipping address
			remove_filter('sc_substep_shipping_address_text', array($this, 'add_shipping_phone_to_substep_text_format'), 10);

			// Add shipping phone to contact fields
			add_filter('sc_checkout_contact_step_field_ids', array($this, 'add_shipping_phone_field_to_contact_fields'), 10);
			add_filter('sc_substep_contact_text', array($this, 'add_shipping_phone_to_substep_text_format'), 10);
		}
	...
	{
		return SimpleCheckout_CheckoutFields::instance();
	}
	...
	{
		return apply_filters('sc_shipping_phone_field_args', array(
			'label'        => __('Shipping phone', 'simple-checkout'),
			'description'  => __('Only used for shipping-related questions.', 'simple-checkout'),
			'required'     => get_option('sc_shipping_phone_field_visibility', 'no') === 'required',
	...
	public function maybe_set_shipping_phone_required($field_args)
	{
		...
		if (!array_key_exists('shipping_phone', $field_args) || get_option('woocommerce_checkout_phone_field', 'required') !== 'required') {
			return $field_args;
		}
	...
	public function change_shipping_company_field_args($field_args)
	{
		...
		if (get_option('sc_enable_checkout_hide_optional_fields', 'yes') === 'yes' && array_key_exists('shipping_phone', $field_args) && array_key_exists('required', $field_args['shipping_phone']) && $field_args['shipping_phone']['required'] != true) {
			return $field_args;
		}
	...
	public function add_shipping_phone_field($fields)
	{
		...
		foreach ($field_args as $field => $values) {
			if (array_key_exists($field, $fields)) {
				$fields[$field] = array_merge($fields[$field], $values);
			}
		}
...
SimpleCheckout_CheckoutShippingPhoneField::instance();
