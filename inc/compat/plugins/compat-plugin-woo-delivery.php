 ...
 */
class SimpleCheckout_WooDelivery extends SimpleCheckout
{
	...
	public function late_hooks()
	{
		if (class_exists('Coderockz_Woo_Delivery') && class_exists('Coderockz_Woo_Delivery_Public')) {
			...
			$substep_position_priority = array(
				'before_billing' => array('sc_output_step_billing', 9),
				'after_billing' => array('sc_output_step_billing', 11),
				'before_shipping' => array('sc_output_step_shipping', 9),
				'after_shipping' => array('sc_output_step_shipping', 11),
				'before_notes' => array('sc_output_step_shipping', 99),
				'after_notes' => array('sc_output_step_shipping', 101),
				'before_payment' => array('sc_output_step_payment', 89), // Intentionally same as for `before_your_order`
				'before_your_order' => array('sc_output_step_payment', 89),
			);
			...
			add_filter('sc_is_step_complete_shipping', array($this, 'maybe_set_step_incomplete'), 10);
		}
	}
	...
	public function output_substep_delivery_options($step_id)
	{
		...
		$substep_title = __('Delivery Options', 'simple-checkout');
		if ($delivery_option_settings['enable_option_time_pickup'] === true) {
			...
			if ($delivery_date_settings['enable_delivery_date'] === true && $pickup_date_settings['enable_pickup_date'] === true) {
				$substep_title = __('Delivery or Pickup', 'simple-checkout');
			}
			// Only delivery enabled
			else if ($delivery_date_settings['enable_delivery_date'] === true) {
				$substep_title = __('Delivery Date', 'simple-checkout');
			}
			// Only pickup enabled
			else if ($pickup_date_settings['enable_pickup_date'] === true) {
				$substep_title = __('Pickup Date', 'simple-checkout');
			}
		}
		$substep_title = apply_filters('sc_woodelivery_substep_title', $substep_title);
	...
	public function get_substep_text_delivery_options()
	{
		...
		return apply_filters('sc_substep_delivery_date_text', $html);
	}
...
SimpleCheckout_WooDelivery::instance();
