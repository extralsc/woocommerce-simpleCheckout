 ...
 */
class SimpleCheckout_MapToAddress extends SimpleCheckout
{
	...
	public function late_hooks()
	{
		if (class_exists('Sgitsdlmp_Public')) {
			...
			remove_action('woocommerce_before_checkout_billing_form', array(SimpleCheckout_Steps::instance(), 'output_billing_same_as_shipping_field'), 100);

			// Re-add hooks in different position
			$plugin_public = new Sgitsdlmp_Public('map-to-address', SGITSDLMP_VERSION);
			add_action('sc_after_substep_billing_address', array($plugin_public, 'sgitsdlmp_woocommerce_after_checkout_billing_form'), 10);
			add_action('sc_after_substep_shipping_address', array($plugin_public, 'sgitsdlmp_woocommerce_after_checkout_shipping_form'), 10);
			...
			// Remove substep text
			add_filter('sc_substep_billing_address_text', '__return_empty_string');
			add_filter('sc_substep_shipping_address_text', '__return_empty_string');
		}
	}
...
SimpleCheckout_MapToAddress::instance();
