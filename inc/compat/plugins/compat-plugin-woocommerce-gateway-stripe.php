 ...
 */
class SimpleCheckout_PaymentMethodStripe extends SimpleCheckout
{
		...
		if (class_exists('WC_Stripe_Payment_Request')) {
			...
			if ('yes' === apply_filters('sc_woocommerce_gateway_stripe_show_buttons', 'yes') && is_array(WC_Stripe_Payment_Request::instance()->stripe_settings) && array_key_exists('payment_request', WC_Stripe_Payment_Request::instance()->stripe_settings) && 'yes' === WC_Stripe_Payment_Request::instance()->stripe_settings['payment_request'] && WC_Stripe_Payment_Request::instance()->should_show_payment_request_button()) {
				...
				if (version_compare($stripe_plugin_version, '5.5.0', '<')) {
					...
					add_action('sc_checkout_express_checkout', array(WC_Stripe_Payment_Request::instance(), 'display_payment_request_button_html'), 10);
				}
				// Versions 5.5.0+
...
SimpleCheckout_PaymentMethodStripe::instance();
