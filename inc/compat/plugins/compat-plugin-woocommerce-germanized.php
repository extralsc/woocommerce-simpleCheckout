 ...
 */
class SimpleCheckout_WooCommerceGermanized extends SimpleCheckout
{
	...
	public function late_hooks()
	{
		...
		if (function_exists('wc_gzd_get_hook_priority')) {
			...
			$place_order_position = get_option('sc_integration_woocommerce_germanized_place_order_position', 'order_summary_after_total');
			...
			$legal_checkboxes_position = 'payment_step' === $place_order_position ? 'before_place_order' : get_option('sc_integration_woocommerce_germanized_legal_checkboxes_position', 'order_summary_before_products');
	...
	public function add_settings($settings, $current_section)
	{
		...
		if (true !== apply_filters('sc_integration_woocommerce_germanized_settings', false)) {
			return $settings;
		}

		$settings[] = array(
			'title'          => __('Germanized for WooCommerce', 'simple-checkout'),
			'desc'           => __('Define the position to display the place order button. <br/><span style="color:#f00;"><strong>Disclaimer:</strong> by changing these settings, I confirm that I understand the legal implications of changing the position of the legal checkboxes on my checkout page.</span>', 'simple-checkout'),
			'id'             => 'sc_integration_woocommerce_germanized_place_order_position',
			'options'        => array(
				'order_summary_after_total'      => _x('Order summary (after totals)', 'Place order position', 'simple-checkout'),
				'payment_step'                   => _x('Payment step', 'Place order position', 'simple-checkout'),
			),
		...
		$settings[] = array(
			'desc'           => __('Define the position to display the legal checkboxes. <br/>The legal checkboxes will be displayed in the payment section if the place order button is set to display in the payment section.<br/><span style="color:#f00;"><strong>Disclaimer:</strong> by changing these settings, I confirm that I understand the legal implications of changing the position of the legal checkboxes on my checkout page.</span>', 'simple-checkout'),
			'id'             => 'sc_integration_woocommerce_germanized_legal_checkboxes_position',
			'options'        => array(
				'order_summary_before_products'  => _x('Order summary (before products)', 'Legal checkboxes position', 'simple-checkout'),
				'before_place_order'             => _x('Before place order button', 'Legal checkboxes position', 'simple-checkout'),
			),
...
SimpleCheckout_WooCommerceGermanized::instance();
