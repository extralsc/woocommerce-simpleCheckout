 ...
 */
class SimpleCheckout_Steps extends SimpleCheckout
{
	...
	public function hooks()
	{
		...
		add_action('sc_output_custom_styles', array($this, 'output_checkout_header_custom_styles'), 10);
		add_action('sc_output_custom_styles', array($this, 'output_checkout_page_custom_styles'), 10);
		...
		// Checkout Header
		add_action('sc_checkout_header', array($this, 'output_checkout_header'), 1);
		add_action('sc_checkout_header_cart_link', array($this, 'output_checkout_header_cart_link'), 10);
		add_filter('woocommerce_update_order_review_fragments', array($this, 'add_checkout_header_cart_link_fragment'), 10);

		// Container class
		add_filter('sc_content_section_class', array($this, 'sc_content_section_class'), 10);
		...
		add_action('sc_checkout_steps', array($this, 'output_checkout_steps'), 10);
		add_action('sc_checkout_after', array($this, 'output_checkout_sidebar_wrapper'), 10);
		...
		add_action('sc_output_step_contact', array($this, 'output_substep_contact_login'), 10);
		add_action('sc_output_step_contact', array($this, 'output_substep_contact'), 20);
		...
		// Account creation
		add_action('sc_checkout_after_contact_fields', array($this, 'output_form_account_creation'), 10);

		// Shipping
		add_filter('option_woocommerce_ship_to_destination', array($this, 'change_woocommerce_ship_to_destination'), 100, 2);
		add_action('sc_output_step_shipping', array($this, 'output_substep_shipping_address'), 10);
		add_action('sc_output_step_shipping', array($this, 'output_substep_shipping_method'), 20);
		add_action('sc_before_checkout_shipping_address_wrapper', array($this, 'output_ship_to_different_address_hidden_field'), 10);
		...
		// Billing Address
		add_action('sc_output_step_billing', array($this, 'output_substep_billing_address'), 10);
		...
		add_action('sc_checkout_payment', 'woocommerce_checkout_payment', 20);
		add_action('sc_output_step_payment', array($this, 'output_substep_payment'), 80);
		add_action('sc_output_step_payment', array($this, 'output_order_review'), 90);
		add_action('sc_output_step_payment', array($this, 'output_checkout_place_order'), 100, 2);
		add_action('sc_checkout_after_order_review_inside', array($this, 'output_checkout_place_order_for_sidebar'), 1);
		...
		// Order Review
		add_action('sc_checkout_order_review_section', array($this, 'output_order_review_for_sidebar'), 10);
		add_action('sc_review_order_shipping', array($this, 'maybe_output_order_review_shipping_method_chosen'), 30);
	...
	public function very_late_hooks()
	{
		...
		if (!function_exists('is_checkout') || (!is_checkout() && !is_cart() && (!defined('DOING_AJAX') || !DOING_AJAX))) {
			return;
		}
		...
		if (in_array('order', array_keys($all_fields))) {
			...
			$order_notes_substep_position = 'sc_output_step_shipping';
			...
			if (apply_filters('woocommerce_enable_order_notes_field', 'yes' === get_option('woocommerce_enable_order_comments', 'yes')) && is_array($additional_order_fields) && count($additional_order_fields) > 0) {
				...
				if (!WC()->cart->needs_shipping()) {
					$order_notes_substep_position = 'sc_output_step_billing';
				}
		...
		if (!in_array('order', array_keys($all_fields)) || !apply_filters('woocommerce_enable_order_notes_field', 'yes' === get_option('woocommerce_enable_order_comments', 'yes'))) {
			$order_notes_substep_position = apply_filters('sc_do_order_notes_hooks_position', 'sc_checkout_after_step_shipping_fields');
			$order_notes_substep_priority = apply_filters('sc_do_order_notes_hooks_priority', 100);
	...
	public function add_custom_styles()
	{
		...
		if (!has_action('sc_output_custom_styles')) {
			return;
		}
?>
	...
	public function output_checkout_header_custom_styles()
	{
		...
		$header_background_color = trim(get_option('sc_checkout_header_background_color', ''));
	...
	public function output_checkout_page_custom_styles()
	{
		...
		// Get header background color
		$page_background_color = trim(get_option('sc_checkout_page_background_color', ''));
		...
		echo 'body.has-simple-checkout{background-color:' . $page_background_color . '}';
	}
	...
	public function get_hide_site_header_footer_at_checkout()
	{
		...
		return (!(!$checkout->is_registration_enabled() && $checkout->is_registration_required() && !is_user_logged_in())) && 'yes' === get_option('sc_hide_site_header_footer_at_checkout', 'yes');
	}
	...
	{
		return apply_filters('sc_allowed_checkout_layouts', array(
			'multi-step' => __('Multi-step', 'simple-checkout'),
			'single-step' => __('Single step', 'simple-checkout'),
		));
	}
	...
	public function get_checkout_layout()
	{
		...
		$current_value = get_option('sc_checkout_layout');
		...
		return apply_filters('sc_get_checkout_layout', $current_value);
	}
	...
	{
		return apply_filters('sc_is_checkout_layout_multistep', $this->get_checkout_layout() === 'multi-step');
	}
	 ...
	 */
	public function sc_content_section_class($class)
	{
	...
	public function get_next_step_button_label($step_id)
	{
		...
		return sprintf(__('Proceed to %s', 'simple-checkout'), $next_step_args['step_title']);
	}
	...
	public function register_checkout_step($step_args)
	{
		...
		$step_args = apply_filters('sc_register_checkout_step_args', $step_args);
	...
	public function register_default_checkout_steps()
	{
		...
		if (count($this->checkout_steps) > 0) {
			return;
		}
		...
		$this->register_checkout_step(array(
			'step_id' => 'contact',
			'step_title' => _x('Contact', 'Checkout step title', 'simple-checkout'),
			'priority' => 10,
		...
		if (WC()->cart->needs_shipping()) {
			$this->register_checkout_step(array(
				'step_id' => 'shipping',
				'step_title' => _x('Shipping', 'Checkout step title', 'simple-checkout'),
				'priority' => 20,
		...
		$this->register_checkout_step(array(
			'step_id' => 'billing',
			'step_title' => _x('Billing', 'Checkout step title', 'simple-checkout'),
			'priority' => 30,
		...
		$this->register_checkout_step(array(
			'step_id' => 'payment',
			'step_title' => _x('Payment', 'Checkout step title', 'simple-checkout'),
			'priority' => 100,
	...
	public function output_checkout_steps()
	{
		foreach ($this->get_checkout_steps() as $step_index => $step_args) {
			...
			if ($render_conditional_callback && is_callable($render_conditional_callback) && !call_user_func($render_conditional_callback)) {
				continue;
			}
	...
	public function output_checkout_progress_bar()
	{
		...
		// Filter attributes
		$progress_bar_attributes = apply_filters('sc_checkout_progress_bar_attributes', $progress_bar_attributes);
		$progress_bar_inner_attributes = apply_filters('sc_checkout_progress_bar_inner_attributes', $progress_bar_inner_attributes);
	...
	public function output_substep_start_tag($step_id, $substep_id, $substep_title, $additional_attributes = array())
	{
		...
		do_action("sc_before_substep_{$substep_id}", $step_id, $substep_id);
	}
	...
	{
		do_action("sc_after_substep_{$substep_id}", $step_id, $substep_id, $output_edit_buttons);
		?>
	...
	<?php
			...
			{
				do_action('sc_output_step_contact', 'contact');
			}
			...
			{
				$substep_id = 'contact';
				$substep_title = __('My contact', 'simple-checkout');
				$this->output_substep_start_tag($step_id, $substep_id, $substep_title, apply_filters("sc_substep_{$substep_id}_attributes", array()));
			...
			public function get_substep_text_contact()
			{
				...
				return apply_filters('sc_substep_contact_text', $html);
			}
			...
			public function is_step_complete_contact()
			{
				...
				return apply_filters('sc_is_step_complete_contact', $is_step_complete);
			}
			...
			{
				return apply_filters('sc_checkout_contact_step_field_ids', array(
					'billing_email',
				));
			}
			...
			public function output_substep_contact_login($step_id)
			{
				...
				$this->output_substep_start_tag($step_id, $substep_id, null, apply_filters("sc_substep_{$substep_id}_attributes", array()));
			...
			{
				do_action('sc_output_step_shipping', 'shipping');
			}
			...
			public function output_substep_shipping_address($step_id)
			{
				...
				$substep_title = __('Shipping to', 'simple-checkout');
				$this->output_substep_start_tag($step_id, $substep_id, $substep_title, apply_filters("sc_substep_{$substep_id}_attributes", array()));
			...
			public function output_substep_shipping_method($step_id)
			{
				...
				$substep_title = __('Shipping method', 'simple-checkout');
				$this->output_substep_start_tag($step_id, $substep_id, $substep_title, apply_filters("sc_substep_{$substep_id}_attributes", array()));
			...
			public function output_substep_order_notes($step_id)
			{
				...
				$substep_title = __('Additional notes', 'simple-checkout');
				$this->output_substep_start_tag($step_id, $substep_id, $substep_title, apply_filters("sc_substep_{$substep_id}_attributes", array()));
			...
			public function get_substep_text_shipping_method()
			{
				...
				foreach ($packages as $i => $package) {
					...
					$chosen_method_label = $method ? wc_cart_totals_shipping_method_label($method) : __('Not selected yet.', 'simple-checkout');
				...
				return apply_filters('sc_substep_shipping_methods_text', $html);
			}
			...
			public function get_substep_text_order_notes()
			{
				...
				return apply_filters('sc_substep_order_notes_text', $html);
			}
			...
			public function is_step_complete_shipping()
			{
				...
				return apply_filters('sc_is_step_complete_shipping', $is_step_complete);
			}
	...
	?>
		...
		<?php
			...
			public function get_shipping_methods_available()
			{
				...
				do_action('sc_shipping_methods_after_packages');
			...
			{
				$label     = sprintf(apply_filters('sc_shipping_method_option_label_markup', '<span class="shipping-method__option-text">%s</span>'), $method->get_label());
				$has_cost  = 0 < $method->cost;
				$hide_cost = !$has_cost && in_array($method->get_method_id(), array('free_shipping', 'local_pickup'), true);

				// Maybe add shipping method description
				$method_description = apply_filters('sc_shipping_method_option_description', '', $method);
				$method_description_markup = !empty($method_description) ? apply_filters('sc_shipping_method_option_description_markup', ' <span class="shipping-method__option-description">%s</span>') : '';
				...
				if ($has_cost && !$hide_cost) {
					...
					if (WC()->cart->display_prices_including_tax()) {
						...
						$label .= sprintf(apply_filters('sc_shipping_method_option_price_markup', ' <span class="shipping-method__option-price">%s</span>'), $method_costs);
					} else {
			...
			{
				do_action('sc_output_step_billing', 'billing');
			}
			...
			public function output_substep_billing_address($step_id)
			{
				...
				$substep_title = __('Billing to', 'simple-checkout');
				$this->output_substep_start_tag($step_id, $substep_id, $substep_title, apply_filters("sc_substep_{$substep_id}_attributes", array()));
			...
			public function output_substep_billing_address_fields()
			{
				...
				do_action('sc_checkout_before_step_billing_fields');
				...
				do_action('sc_checkout_after_step_billing_fields');
			}
			...
			public function get_substep_text_billing_address()
			{
				...
				return apply_filters('sc_substep_billing_address_text', $html);
			}
			...
			public function is_step_complete_billing()
			{
				...
				foreach ($fields as $field_key => $field) {
					...
					if (in_array($field_key, $contact_display_field_keys)) {
						continue;
					}
				...
				return apply_filters('sc_is_step_complete_billing', $is_step_complete);
			}
		...
		?>
			<p id="billing_same_as_shipping_field" class="form-row form-row-wide">
				...
				<label for="billing_same_as_shipping"><?php echo esc_html(__('Same as shipping address', 'simple-checkout')); ?></label>
			</p>
		<?php
				endif;
			}
			...
			public function is_billing_same_as_shipping_checked()
			{
				...
				// Set default value
				$billing_same_as_shipping = apply_filters('sc_default_to_billing_same_as_shipping', get_option('sc_default_to_billing_same_as_shipping', 'yes') == 'yes');
				...
				else if (WC()->session->__isset('sc_billing_same_as_shipping')) {
					$billing_same_as_shipping = WC()->session->get('sc_billing_same_as_shipping') === '1';
				}
			...
			{
				// Set session value
				WC()->session->set('sc_billing_same_as_shipping', $billing_same_as_shipping ? '1' : '0');
			}
			...
			public function get_billing_same_shipping_fields_keys()
			{
				...
				$skip_field_keys = apply_filters('sc_billing_same_as_shipping_skip_fields', array());
				...
				return apply_filters('sc_billing_same_as_shipping_field_keys', $billing_copy_shipping_field_keys);
			}
			...
			{
				do_action('sc_output_step_payment', 'payment');
			}
			...
			{
				$substep_id = 'payment';
				$substep_title = __('Payment method', 'simple-checkout');
				$this->output_substep_start_tag($step_id, $substep_id, $substep_title, apply_filters("sc_substep_{$substep_id}_attributes", array()));
			...
			public function output_checkout_sidebar_wrapper()
			{
				...
				// Sticky state attributes
				if (get_option('sc_enable_checkout_sticky_order_summary', 'yes') === 'yes') {
				...
				// Filter attributes
				$sidebar_attributes = apply_filters('sc_checkout_sidebar_attributes', $sidebar_attributes);
				$sidebar_attributes_inner = apply_filters('sc_checkout_sidebar_attributes_inner', $sidebar_attributes_inner);
		...
		<div <?php echo $sidebar_attributes_str; // WPCS: XSS ok. 
			...
			<div <?php echo $sidebar_attributes_inner_str; // WPCS: XSS ok. 
					?>>
				<?php do_action('sc_checkout_order_review_section'); ?>
			</div>
		</div>
<?php
			}
			...
			{
				return apply_filters('sc_order_review_title', __('Order summary', 'simple-checkout'));
			}
			...
			public function get_order_review_html_attributes($is_sidebar_widget = false)
			{
				...
				if (get_option('sc_enable_checkout_place_order_sidebar', 'no') === 'yes' || is_active_sidebar('sc_order_summary_after')) {
			...
			public function output_checkout_place_order_for_sidebar($is_sidebar_widget)
			{
				...
				if (get_option('sc_enable_checkout_place_order_sidebar', 'no') === 'no') {
					return;
				}
			...
			public function get_customer_session_field_keys()
			{
				...
				$skip_field_keys = apply_filters('sc_customer_persisted_data_skip_fields', $skip_field_keys, $parsed_posted_data);
			...
			public function unset_session_customer_persisted_data_order_processed()
			{
				...
				$clear_field_keys = apply_filters('sc_customer_persisted_data_clear_fields_order_processed', $clear_field_keys);
			...
			public function unset_all_session_customer_persisted_data()
			{
				...
				$clear_field_keys_skip_list = apply_filters('sc_customer_persisted_data_clear_all_fields_skip_list', array('order_comments'));
				...
				foreach ($clear_field_keys as $field_key) {
					...
					if (in_array($field_key, $clear_field_keys_skip_list)) {
						continue;
					}
