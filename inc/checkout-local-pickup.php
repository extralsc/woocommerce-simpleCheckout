 ...
 */
class SimpleCheckout_CheckoutLocalPickup extends SimpleCheckout
{
	...
	public function very_late_hooks()
	{
		...
		if ($this->is_local_pickup_available()) {
			remove_action('sc_output_step_shipping', array(SimpleCheckout_Steps::instance(), 'output_substep_shipping_address'), 10);
			remove_action('sc_output_step_shipping', array(SimpleCheckout_Steps::instance(), 'output_substep_shipping_method'), 20);
			add_action('sc_output_step_shipping', array(SimpleCheckout_Steps::instance(), 'output_substep_shipping_method'), 10);
			add_action('sc_output_step_shipping', array(SimpleCheckout_Steps::instance(), 'output_substep_shipping_address'), 20);
			add_filter('sc_substep_shipping_address_attributes', array($this, 'change_substep_attributes_shipping_address'), 10);
			add_action('sc_checkout_after_step_shipping_fields', array($this, 'output_substep_state_hidden_fields_shipping_fields'), 10);
			add_action('sc_checkout_after_step_shipping_fields', array($this, 'maybe_output_shipping_address_text'), 10);
			add_filter('woocommerce_cart_needs_shipping_address', array($this, 'maybe_change_needs_shipping_address'), 10);
			add_filter('sc_substep_title_shipping_address', array($this, 'maybe_change_shipping_address_substep_title'), 50);
			add_filter('woocommerce_update_order_review_fragments', array($this, 'add_shipping_address_substep_title_fragment'), 10);
			add_filter('sc_substep_shipping_address_text', array($this, 'change_substep_text_shipping_address'), 50);
		}
	}
	...
	public function maybe_output_shipping_address_text()
	{
		...
		SimpleCheckout_Steps::instance()->output_substep_text_shipping_address();
	}
	...
	public function is_shipping_method_local_pickup_selected()
	{
		...
		return apply_filters('sc_is_shipping_method_local_pickup_selected', $is_shipping_method_local_pickup_selected);
	}
	...
	public function is_local_pickup_available()
	{
		...
		return apply_filters('sc_is_local_pickup_available', $is_local_pickup_available);
	}
	...
	public function maybe_change_shipping_address_substep_title($substep_title)
	{
		...
		if ($this->is_shipping_method_local_pickup_selected()) {
			$substep_title = apply_filters('sc_shipping_address_local_pickup_point_title', __('Pickup point', 'simple-checkout'));
		}
...
SimpleCheckout_CheckoutLocalPickup::instance();
