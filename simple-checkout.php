...
register_activation_hook(__FILE__, array('SimpleCheckout_Activation', 'on_activation'));
...
class SimpleCheckout
{
	...
	public static $plugin = 'Simple Checkout';
	public static $plugin_slug = 'simple-checkout';
	...
	{
		load_plugin_textdomain('simple-checkout', false, 'simple-checkout/languages');
	}
	...
	public function hooks()
	{
		...
		if (!$this->is_woocommerce_activated()) {
			add_action('sc_admin_notices', array($this, 'add_woocommerce_required_notice'), 10);
			return;
		}
	...
	private function add_features()
	{
		self::$features = array(
			...
			'checkout-page-template'              => array('file' => self::$directory_path . 'inc/checkout-page-template.php', 'enable_option' => 'sc_enable_checkout_page_template', 'enable_default' => 'yes'),

			'checkout-fields'                     => array('file' => self::$directory_path . 'inc/checkout-fields.php', 'enable_option' => 'sc_apply_checkout_field_args', 'enable_default' => 'yes'),
			'checkout-hide-optional-fields'       => array('file' => self::$directory_path . 'inc/checkout-hide-optional-fields.php', 'enable_option' => 'sc_enable_checkout_hide_optional_fields', 'enable_default' => 'yes'),
			'checkout-shipping-phone'             => array('file' => self::$directory_path . 'inc/checkout-shipping-phone-field.php', 'enable_option' => 'sc_shipping_phone_field_visibility', 'enable_default' => 'no'),
			'checkout-express-checkout'           => array('file' => self::$directory_path . 'inc/checkout-express-checkout.php', 'enable_option' => 'sc_enable_checkout_express_checkout', 'enable_default' => 'yes'),
			'checkout-local-pickup'               => array('file' => self::$directory_path . 'inc/checkout-local-pickup.php', 'enable_option' => 'sc_enable_checkout_local_pickup', 'enable_default' => 'yes'),
			'checkout-validation'                 => array('file' => self::$directory_path . 'inc/checkout-validation.php', 'enable_option' => 'sc_enable_checkout_validation', 'enable_default' => 'yes'),
			'checkout-gift-options'               => array('file' => self::$directory_path . 'inc/checkout-gift-options.php', 'enable_option' => 'sc_enable_checkout_gift_options', 'enable_default' => 'no'),
			'checkout-coupon-codes'               => array('file' => self::$directory_path . 'inc/checkout-coupon-codes.php', 'enable_option' => 'sc_enable_checkout_coupon_codes', 'enable_default' => 'yes'),
			'checkout-widget-areas'               => array('file' => self::$directory_path . 'inc/checkout-widget-areas.php', 'enable_option' => 'sc_enable_checkout_widget_areas', 'enable_default' => 'yes'),
			'packing-slips'                       => array('file' => self::$directory_path . 'inc/packing-slips.php', 'enable_option' => 'sc_enable_packing_slips_options', 'enable_default' => 'yes'),
		);
	}
	...
	public function get_assets_version_number()
	{
		...
		$min = get_option('sc_load_unminified_assets', 'no') === 'yes' ? '' : '.min';
	...
	public function locate_template($template, $template_name, $template_path)
	{
		...
		if (!$_template || apply_filters('sc_override_template_with_theme_file', false, $template, $template_name, $template_path)) {
	...
	public function load_features()
	{
		...
		$_features = apply_filters('sc_init_features_list', self::$features);
	...
	public function load_plugin_compat_features()
	{
		...
		foreach ($plugins_installed as $plugin_file => $plugin_meta) {
			...
			if (get_option('sc_enable_compat_plugin_' . $plugin_slug, true) === 'false') {
				continue;
			}
	...
	public function load_theme_compat_features()
	{
		...
		foreach ($theme_slugs as $theme_slug) {
			// Maybe skip compat file
			if (get_option('sc_enable_compat_theme_' . $theme_slug, true) === 'false') {
				continue;
			}
	...
	public function add_woocommerce_required_notice($notices = array())
	{
		$notices[] = array(
			...
			'error' => true,
			'description' => sprintf(wp_kses_post(__('<strong>%1$s</strong> requires <strong>%2$s</strong> to be installed and activated. <a href="%3$s">Go to Plugin Search</a>', 'simple-checkout')), self::$plugin, 'WooCommerce', admin_url('plugin-install.php?s=woocommerce&tab=search&type=term')),
			'dismissable' => false,
		);
		return $notices;
	}
	...
	public function get_hooked_function_for_class($tag, $class_name, $priority)
	{
		...
		if (!is_array($callbacks) || !array_key_exists($priority, $callbacks)) {
			return false;
		}
	...
	public function remove_filter_for_class($tag, $function_array, $priority)
	{
		...
		if (!is_array($function_array) || count($function_array) < 2 || !is_string($function_array[0]) || !is_string($function_array[1])) {
			return false;
		}
