...
$gift_message_fields = array_filter($gift_options_fields, function ($key) {
	return in_array($key, SimpleCheckout_GiftOptions::instance()->get_gift_message_field_ids());
...
<div id="sc-gift-options">
	...
	$has_required_message = array_key_exists('_sc_gift_message', $gift_message_fields) && array_key_exists('required', $gift_message_fields['_sc_gift_message']) && ($gift_message_fields['_sc_gift_message']['required'] === true);
	if (!empty($gift_options['_sc_gift_message']) || $has_required_message || get_option('sc_default_gift_options_expanded', 'no') === 'yes') {
		$expansible_section_args['initial_state'] = 'expanded';
	}

	// Output gift options expansible form section
	SimpleCheckout_Steps::instance()->output_expansible_form_section_start_tag('gift_options', apply_filters('sc_expansible_section_toggle_label_gift_options', __('Add a gift message', 'simple-checkout')), $expansible_section_args);
	...
	SimpleCheckout_Steps::instance()->output_expansible_form_section_end_tag();

	do_action('sc_checkout_gift_options_after_fields');
	?>
