...
<header class="sc-checkout-header">
	<div class="sc-checkout-header__inner">
		...
		<div class="sc-checkout__branding">
			...
			if (!empty(get_option('sc_checkout_logo_image', ''))) {
				echo sprintf(
					'<a href="%1$s" class="custom-logo-link" rel="home">%2$s</a>',
					esc_url(home_url('/')),
					wp_get_attachment_image(get_option('sc_checkout_logo_image', ''), 'full')
				);
			} else if (has_action('sc_checkout_header_logo')) {
				do_action('sc_checkout_header_logo');
