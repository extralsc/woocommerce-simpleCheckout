...
<div class="shipping shipping-method__package" data-title="<?php echo esc_attr($package_name); ?>" data-package-index="<?php echo esc_attr($package_index); ?>">
	...
	?>
	<?php if (true === apply_filters('sc_shipping_method_display_package_name', false)) : ?>
		<p class="shipping-method__package-name"><?php echo esc_html($package_name); ?></p>
	<?php endif; ?>

	<?php if (count($available_methods) > 0) : ?>

		<?php echo apply_filters('sc_shipping_method_option_start_tag_markup', '<ul id="shipping_method" class="shipping-method__options">'); ?>
		...
		foreach ($available_methods as $method) :
			...
			echo apply_filters(
				'sc_shipping_method_option_markup',
				sprintf(
					'<li class="shipping-method__option"><input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method" %4$s />
					...
					SimpleCheckout_Steps::instance()->get_cart_shipping_methods_label($method)
				),
				$method,
				$package_index,
				$chosen_method,
		...
		<?php echo apply_filters('sc_shipping_method_option_end_tag_markup', '</ul>'); ?>
